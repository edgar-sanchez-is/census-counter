		...
		if (strncmp(command, "BUY", 3) == 0) {
			...
			for(int i = 0; i <= 19; ++i) {
				if (ticket[i].status == true) {
					printf("[SERVER X] : Client %zu bought %i\n", clientNum, ticket[i].number);
			...
			for(int i = 0; i <= 19; ++i) {
				if (ticket[i].number == atoi(returnNum) && ticket[i].client == clientNum && ticket[i].status == false) {
					...
					printf("[SERVER X] : Client %zu does not own %i\n", clientNum, ticket[i].number);
			...
			fprintf(stderr, "[CLIENT %zu] : Unknown command: %s", clientNum, command);
